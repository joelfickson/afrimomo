...
export class PawapayRefunds extends BaseService {
   ...
   */
  async createRefundRequest(refundData: { refundId: string; depositId: string }): Promise<RefundResponse | NetworkResponse> {
    try {
      ...
      logger.info("Sending refund request for deposit:", refundData.depositId, "with refundId:", refundData.refundId);
   ...
   */
  async getRefundStatus(refundId: string): Promise<RefundTransaction | NetworkResponse> {
    try {
