...
export class PayChangu extends BaseService {
  ...
  async initiatePayment(
    ...
    try {
      ...
      const response = await this.networkManager.initiatePayment(paychanguPurchaseData);
   ...
   */
  async getTransactionById(transactionId: string): Promise<PayChanguTransactionResponse> {
    try {
      logger.info("PayChangu: getting transaction by ID", { transactionId });

      const verificationResponse = await this.networkManager.verifyPayment(transactionId);
