...
// Send bulk payouts
router.post('/payouts/bulk', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const requiredFields = ['payoutId', 'amount', 'currency', 'correspondent', 'recipient', 'customerTimestamp', 'statementDescription'];
    const invalidTransactions = transactions.map((transaction, index) => {
      ...
      if (transaction.recipient?.type !== 'MSISDN' || !transaction.recipient?.address?.value) {
        return {
          index,
          error: 'Invalid recipient structure. Must include type: "MSISDN" and address.value'
        };
      }
      return null;
    }).filter(Boolean);
