...
export class PawapayPayouts extends BaseService {
   ...
   */
  async sendPayout(transaction: PayoutTransaction): Promise<PawaPayPayoutTransaction | NetworkResponse> {
    try {
      const phoneNumber = this.formatPhoneNumber(transaction.phoneNumber);

      logger.info("Sending payout to", phoneNumber, "the amount of", transaction.amount, 
   ...
   */
  async sendBulkPayout(transactions: PayoutTransaction[]): Promise<PawaPayPayoutTransaction[] | NetworkResponse> {
    try {
   ...
   */
  async getPayout(depositId: string): Promise<PawaPayPayoutTransaction | NetworkResponse> {
    try {
