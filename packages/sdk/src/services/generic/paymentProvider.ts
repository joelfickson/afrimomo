...
export class PaymentProviderAdapter {
   ...
   */
  async createPayment(request: GenericPaymentRequest): Promise<GenericPaymentResponse> {
    try {
      ...
      logger.info(`${this.config.name}: Creating payment`, { amount: request.amount, currency: request.currency });
   ...
   */
  async getTransaction(transactionId: string): Promise<GenericTransaction | null> {
    try {
      logger.info(`${this.config.name}: Getting transaction`, { transactionId });
  ...
  async getBalance(): Promise<{ balance: string, currency: string } | null> {
    ...
    try {
      ...
      // Transform the response if a transformer is provided
      return this.config.responseTransformer 
        ? this.config.responseTransformer<{ balance: string, currency: string }>(response)
  ...
  async request<T>(
    ...
    try {
      logger.info(`${this.config.name}: Making custom ${method.toUpperCase()} request to ${endpoint}`);
      
      let response: T | null = null;
      
      switch (method) {
        case 'get':
          response = await this.network.get<T>(endpoint, config || {}, `custom ${method} request`);
          break;
        case 'post':
          response = await this.network.post<T>(endpoint, data || {}, config || {}, `custom ${method} request`);
          break;
        case 'put':
          response = await this.network.put<T>(endpoint, data || {}, config || {}, `custom ${method} request`);
          break;
        case 'delete':
          response = await this.network.delete<T>(endpoint, config || {}, `custom ${method} request`);
          break;
      }
